...
function fetchPostJson(url, objeto, callback) {
    var raiz = document.getElementById("hdfOculto")?.value || "";
    ...
    fetch(urlCompleta, {
        ...
        .catch(err => {
            document.getElementById("divLoading").style.display = "none";
            console.error("❌ Error en fetchPostJson:", err);
...
function GuardarGenericoFormulario(idformulario, tipo) {
    let frm = new FormData(document.getElementById(idformulario));
    let objeto = formToJson(frm);

    // ✅ Si iidpersona está vacío, elimínalo
    if (!objeto.iidpersona || objeto.iidpersona.trim() === "") {
        delete objeto.iidpersona;
    }

    const esNuevo = !objeto.iidpersona;
    let url = esNuevo ? "api/Persona/registrar" : "api/Persona/actualizar";

    fetchPostJson(url, objeto, function (res) {
        if (res === true || res === "true") {
            cerrarModal();
            listar();
            Swal.fire("✅ Éxito", "Se guardó correctamente", "success");
        } else {
            Swal.fire("❌ Error", "Ocurrió un error: " + JSON.stringify(res), "error");
        }
    });
}
...
function GuardarGenericoFormulario(idformulario, tipo) {
    ...
    const esNuevo = !objeto.iidpersona || objeto.iidpersona == 0;
    let url = esNuevo ? "api/Persona/registrar" : "api/Persona/actualizar";
    ...
    fetchPostJson(url, objeto, function (res) {
        if (res) {
            ...
            // El error ya lo muestra fetchPostJson
            console.log("❌ Falló guardar");
        }
    });
}
...
function formToJson(formData) {
    ...
    formData.forEach((value, key) => {
        if (key === "iidpersona" || key === "iidsexo") {
            obj[key] = value ? parseInt(value) : 0;
        } else {
            obj[key] = value;
        }
    });
    console.log("🧾 Datos enviados:", obj); // 👈 importante para depurar
    return obj;
}
...
function EliminarGenericoFormulario() {
    const id = document.getElementsByName("iidpersona")[0].value;
    if (!id) {
        Swal.fire("❌ Error", "No se encontró el ID de la persona.", "error");
        return;
    }

    Swal.fire({
        title: "¿Estás seguro?",
        text: "Esta acción eliminará a la persona seleccionada.",
        icon: "warning",
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            const url = `${objFormularioGlobal.eliminar.url}/${id}`;

            fetchDelete(url, "json", function (res) {
                if (res === 1 || res === true) {
                    cerrarModal();
                    listar();
                    Swal.fire("✅ Éxito", "Persona eliminada correctamente.", "success");
                } else {
